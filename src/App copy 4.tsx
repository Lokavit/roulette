...
type DayLog = {
  energy: Energy;
  rolled: string[]; // today rolled task ids
  selected?: string;
...
export default function App() {
  ...
  function getAvailableTasksForSpin(
    energyLevel: Energy,
    excludeIds: Set<string>
  ) {
    return taskPool.tasks.filter(
      (t) => t.energy === energyLevel && !excludeIds.has(t.id)
    );
  }
  ...
  function rollSpin() {
    ...
    // EXCLUDE today's rolled
    const excludeIds = new Set<string>(prevRolled);

    const available = getAvailableTasksForSpin(energy, excludeIds);

    if (available.length === 0) {
      showToast("该能量等级任务已抽完（今日无可抽任务）");
      return;
    }

    const chosen = pickRandom(available);
    if (!chosen) {
      showToast("抽取失败（available为空）");
      return;
    }
  ...
  const spinDisabled = !yearLog || spinning || cooldown > 0 || rollsUsed >= 3;
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      <div className="mx-auto max-w-5xl px-4 py-10">
        ...
        <div className="mt-6 rounded-2xl border border-zinc-800 bg-zinc-900/40 p-5">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <div className="mt-1 text-sm text-zinc-400">
                每次 Spin 只展示最新结果，但 rolled 会记录历史。
              </div>
            </div>
        ...
        <div className="mt-10 text-xs text-zinc-500">
          <div>Spin 会排除今日已 rolled 的任务。</div>
          <div>
            下一步可以加：跨天 cooldownDays / 任务权重 / 连续多日统计图。
          </div>
        </div>
      </div>
