...
type DayLog = {
  energy: Energy;
  rolled: string[]; // all rolled task ids today
  selected?: string;
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // rolledCandidates should show last rolled task (not all history)
    if (tasks.length > 0) {
      setRolledCandidates([tasks[tasks.length - 1]]);
    } else {
      setRolledCandidates([]);
    }
  ...
  function rollSpin() {
    ...
    // EXCLUDE today's rolled tasks
    const excludeIds = new Set<string>(prevRolled);

    const chosen = pickRandomOneTask(energy, excludeIds);
      ...
      <div className="mx-auto max-w-5xl px-4 py-10">
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-5">
            ...
            <div className="mt-1 text-sm text-zinc-400">
              每次 Spin 只出 1 个任务。并且会排除今日已经抽到过的任务。
            </div>
        ...
        <div className="mt-6 rounded-2xl border border-zinc-800 bg-zinc-900/40 p-5">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <div className="text-lg font-semibold">Spin 结果</div>
              <div className="mt-1 text-sm text-zinc-400">
                每次 Spin 只显示最新抽到的 1 个任务（但日志会记录全部 rolled）。
              </div>
            </div>
          ...
          {todayLog?.rolled && todayLog.rolled.length > 0 && (
        ...
        <div className="mt-10 text-xs text-zinc-500">
          <div>提示：Spin 会排除今日已抽到的任务（rolled 列表）。</div>
          <div>你以后要做“跨天 cooldownDays”过滤，也可以在这里扩展。</div>
        </div>
      </div>
